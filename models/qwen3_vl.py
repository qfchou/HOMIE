...
class Qwen3VLRetForConditionalGeneration(Qwen3VLForConditionalGeneration):
    ...
    def forward(
        ...
        if pool_mode == 'mean':
            # Mean pooling: average all valid tokens before <emb>
            seq_indices = torch.arange(hidden_states.size(1), device=hidden_states.device).unsqueeze(0)
            before_emb_mask = seq_indices < embed_indices.unsqueeze(1)
            valid_mask = before_emb_mask & (attention_mask.bool())
            masked_hidden = hidden_states * valid_mask.unsqueeze(-1).float()
            embed_features = masked_hidden.sum(dim=1) / valid_mask.sum(dim=1, keepdim=True).float()
        else:
            # Last token before <emb>
        ...
        loss_1to2 = loss_fct(cos_sim, nce_labels)  # embed1 -> embed2
        loss_2to1 = loss_fct(cos_sim.t(), nce_labels)
